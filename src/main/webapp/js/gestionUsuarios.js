...
function cargarUsuarios() {
    ...
    fetch('/proyectoJavaFinal/GestionUsuariosServlet')
...
function mostrarModificarModal(id) {
    fetch(`/proyectoJavaFinal/GestionUsuariosServlet?id=${id}`)
        .then(response => response.json())
        .then(usuario => {
            //console.log(usuario)
...
function eliminarUsuario(id) {
    ...
    if (confirm('¿Está seguro de que desea eliminar este usuario?')) {
        fetch(`/proyectoJavaFinal/GestionUsuariosServlet?id=${id}`, { method: 'DELETE' })
            .then(response => response.json())
            .then(data => {
                console.log(data.exito)
                if (data.exito) {
